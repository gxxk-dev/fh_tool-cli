...
from .crypt_unix import md5_crypt
...
def read_stdin_secret(hint: str) -> str:
    """从 stdin 读取一行密码；交互式终端下先向 stderr 打印提示。

    提示走 stderr 且仅在 tty 下输出，避免 PowerShell 等场景静默等待输入
    被误认为卡死，也不污染 --json 的 stdout 与管道脚本。
    """
    if sys.stdin.isatty():
...
def read_secret_from_args(args: Any) -> PasswordInput:
    ...
    if getattr(args, "password_stdin", False):
        return PasswordInput(read_stdin_secret("请输入目标密码(--password-stdin)，回车确认："), generated=False)
...
def set_su_runtime_password(shell_runner: Any, password: str) -> dict[str, Any]:
    hashed_password = md5_crypt(password, "")
